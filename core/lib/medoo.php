...
class medoo
{
	...
	public function hasError(){
		$err = $this->error();
		return !($err[0] == "00000" && is_null($err[1]));
	}
		...
		)
		{
			...
			foreach($join as $sub_table => $relation)
			{
				...
				if ($match[2] != '' && $match[3] != '')
				{
					...
					$table_join[] = $join_array[ $match[2] ] . ' JOIN "' . $this->t($match[3]) . '" ' . (isset($match[5]) ?  'AS "' . $match[5] . '" ' : '') . $relation;
				}
			}
	...
	public function t($table){ return $this->prefix.$table; }
	...
	public function insert($table, $datas)
	{
		...
		foreach ($datas as $data)
		{
			...
			$this->exec('INSERT INTO "' . $this->t($table) . '" (' . implode(', ', $columns) . ') VALUES (' . implode($values, ', ') . ')');
	...
	public function update($table, $data, $where = null)
	{
		...
		return $this->exec('UPDATE "' . $this->t($table) . '" SET ' . implode(', ', $fields) . $this->where_clause($where));
	}
	public function delete($table, $where)
	{
		return $this->exec('DELETE FROM "' . $this->t($table) . '"' . $this->where_clause($where));
	}
	public function replace($table, $columns, $search = null, $replace = null, $where = null)
	{
		...
		return $this->exec('UPDATE "' . $this->t($table) . '" SET ' . $replace_query . $this->where_clause($where));
	}
